...
export async function POST(req: NextRequest) {
  ...
  const { sessionId, styleName, stylePrompt, photoDataUrl } = await req.json() as {
    sessionId?: string; styleName?: string; stylePrompt?: string; photoDataUrl?: string;
  };
  if (!sessionId || !styleName || !stylePrompt || !photoDataUrl) {
    return NextResponse.json({ error: "sessionId, styleName, stylePrompt, and photoDataUrl are required" }, { status: 400 });
  }
  ...
  try {
    ...
    const { error: dbErr } = await supabase.from("hairstyle_generations_v2").insert({
      session_id: sessionId, user_id: auth.userId, style_name: styleName, storage_path: path, status: "complete",
    });
    if (dbErr) throw dbErr;

    logV2.info("v2_hairstyle_generated", { session_id: sessionId, style: styleName });
    ...
    logV2.error("v2_hairstyle_generate_failed", { session_id: sessionId, style: styleName, message, refusal: isRefusal, quota: isQuota });

    // Best-effort failure record so history isn't silently missing this attempt.
    try {
      await supabase.from("hairstyle_generations_v2").insert({
        session_id: sessionId, user_id: auth.userId, style_name: styleName, storage_path: "", status: "failed",
      });
