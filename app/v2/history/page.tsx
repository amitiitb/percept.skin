...
export default function V2HistoryPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      if (!user) { router.replace("/auth/login?next=/v2/history"); return; }
      ...
      <div style={{ maxWidth: "88rem", margin: "0 auto" }}>
        ...
        {rows.length === 0 ? (
          <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "1.6rem", padding: "6rem 3.2rem", textAlign: "center" }}>
            ...
            <PrimaryButton fullWidth={false} onClick={() => router.push("/v2/scan-prep")}>Start your first analysis →</PrimaryButton>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "1.2rem" }}>
            {rows.map((r) => (
              <div key={r.id} onClick={() => r.status === "complete" && router.push(`/v2/report/${r.id}`)}
